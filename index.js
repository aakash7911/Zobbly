    ...
    try {
        ...
        if(targetUser.followers.includes(myId)) {
            ...
            await new Notification({
                recipient: targetId, sender: myId, type: 'follow',
                message: `${me.name} started following you.`, relatedId: myId
            }).save();
        }
        await targetUser.save(); await me.save();
        res.json({ status: status, followersCount: targetUser.followers.length });
    } catch(e) { res.status(500).json({error: "Error in follow system"}); }
});
...
app.get("/api/notifications", verifyToken, async (req, res) => {
    try {
        const notifs = await Notification.find({ recipient: req.user.id }).populate("sender", "name photo").sort({ createdAt: -1 });
        res.json(notifs);
    } catch(e) { res.status(500).json({error: "Error fetching notifications"}); }
});
...
app.get("/api/chat/conversations", verifyToken, async (req, res) => {
    ...
    } catch(e) { res.status(500).json({error: "Error fetching chats"}); }
});
...
app.post("/api/posts/create", verifyToken, upload.single("postImage"), async (req, res) => {
    try {
        ...
        const notifications = user.followers.map(followerId => ({
            ...
            message: `${user.name} added a new post: "${req.body.content.substring(0, 20)}..."`, relatedId: newPost._id
        }));
        if(notifications.length > 0) await Notification.insertMany(notifications);
        res.json(newPost);
    } catch(e) { res.status(500).json({error: "Error creating post"}); }
});
...
app.post("/api/messages", verifyToken, async (req, res) => {
    try {
        ...
        await new Notification({ recipient: req.body.receiverId, sender: req.user.id, type: 'message', message: `New message from ${sender.name}`, relatedId: req.user.id }).save();
...
app.post("/api/messages/upload", verifyToken, upload.single("chatFile"), async (req, res) => {
    ...
    const sender = await User.findById(req.user.id);
    await new Notification({ recipient: req.body.receiverId, sender: req.user.id, type: 'message', message: `${sender.name} sent a file.`, relatedId: req.user.id }).save();
...
// JOB SEARCH (Using Env Vars)
app.get("/api/jobs/mantiks", async (req, res) => {
    const { what } = req.query; 
    try {
        const options = { method: 'GET', url: 'https://jsearch.p.rapidapi.com/search', params: { query: `${what} in India`, page: '1', num_pages: '1' }, headers: { 'X-RapidAPI-Key': process.env.RAPID_API_KEY, 'X-RapidAPI-Host': 'jsearch.p.rapidapi.com' } };
        const response = await axios.request(options);
        const mappedData = response.data.data.map(job => ({ job_title: job.job_title, company_name: job.employer_name, job_location: job.job_city || job.job_country, job_apply_link: job.job_apply_link, job_description: job.job_description }));
        res.json({ source: "JSearch (Proxy)", data: mappedData });
    } catch (err) { res.json({ source: "System", data: [], error: "Search failed" }); }
});
app.get("/api/jobs/mantiks/company", async (req, res) => {
    const { name } = req.query;
    try {
        const options = { method: 'GET', url: 'https://jsearch.p.rapidapi.com/search', params: { query: `${name} jobs in India`, page: '1', num_pages: '1' }, headers: { 'X-RapidAPI-Key': process.env.RAPID_API_KEY, 'X-RapidAPI-Host': 'jsearch.p.rapidapi.com' } };
        const response = await axios.request(options);
        const mappedData = response.data.data.map(job => ({ job_title: job.job_title, company_name: job.employer_name, job_location: job.job_city || job.job_country, job_apply_link: job.job_apply_link, job_description: job.job_description }));
        res.json({ source: "JSearch (Company Proxy)", data: mappedData });
    } catch (err) { res.json({ source: "System", data: [], error: "No data found" }); }
});
app.get("/api/jobs/adzuna", async (req, res) => {
    try { const { what, where } = req.query; const url = `https://api.adzuna.com/v1/api/jobs/in/search/1?app_id=${process.env.ADZUNA_APP_ID}&app_key=${process.env.ADZUNA_APP_KEY}&results_per_page=20&what=${what}&where=${where}`; const response = await axios.get(url); res.json({ source: "Adzuna", data: response.data.results }); } catch (err) { res.status(500).json({ error: "Adzuna Error" }); }
});
app.get("/api/jobs/jsearch", async (req, res) => {
    try { const { query } = req.query; const options = { method: 'GET', url: 'https://jsearch.p.rapidapi.com/search', params: { query: query, page: '1', num_pages: '1' }, headers: { 'X-RapidAPI-Key': process.env.RAPID_API_KEY, 'X-RapidAPI-Host': 'jsearch.p.rapidapi.com' } }; const response = await axios.request(options); res.json({ source: "JSearch", data: response.data.data }); } catch (err) { res.status(500).json({ error: "JSearch Error" }); }
});
app.get("/api/jobs/google", async (req, res) => {
    try { const { q, location } = req.query; const url = `https://serpapi.com/search.json?engine=google_jobs&q=${q}&location=${location}&api_key=${process.env.SERP_API_KEY}`; const response = await axios.get(url); res.json({ source: "Google", data: response.data.jobs_results }); } catch (err) { res.status(500).json({ error: "SerpApi Error" }); }
});
...
app.put("/api/user/update", verifyToken, async (req, res) => {
    try { await User.findByIdAndUpdate(req.user.id, { name: req.body.name, headline: req.body.headline }); res.json({ message: "Updated" }); } catch(e) { res.status(500).json({error: "Error"}); }
});
app.delete("/api/user/delete", verifyToken, async (req, res) => {
    try { await User.findByIdAndDelete(req.user.id); await Post.deleteMany({ userId: req.user.id }); await Message.deleteMany({ $or: [{ senderId: req.user.id }, { receiverId: req.user.id }] }); res.json({ message: "Deleted" }); } catch(e) { res.status(500).json({error: "Error"}); }
});
app.get("/api/user/backup", verifyToken, async (req, res) => {
    try { const user = await User.findById(req.user.id); const posts = await Post.find({ userId: req.user.id }); const messages = await Message.find({ $or: [{ senderId: req.user.id }, { receiverId: req.user.id }] }); res.json({ user, posts, messages }); } catch(e) { res.status(500).json({error: "Error"}); }
});
app.put("/api/user/block/:id", verifyToken, async (req, res) => {
    try { const user = await User.findById(req.user.id); const targetId = new mongoose.Types.ObjectId(req.params.id); if (user.blockedUsers.includes(targetId)) { user.blockedUsers.pull(targetId); res.json({ message: "Unblocked", status: "unblocked" }); } else { user.blockedUsers.push(targetId); res.json({ message: "Blocked", status: "blocked" }); } await user.save(); } catch(e) { res.status(500).json({error: "Error"}); }
});
